...
?>
    <div class="woocommerce-account-my-memberships">
        <?php
        $userActivePlans = wc_memberships_get_user_active_memberships();
        $userActiveSubscriptions = get_posts(array(
            'numberposts' => -1,
        ...
        $userActiveSubscriptionsCount = count($userActiveSubscriptions);
        if (count($userActivePlans) == 0 || $userActivePlans[0]->plan->slug == 'free-trial' || $userActiveSubscriptionsCount > 0) {
            $plans = (new Pupuga\Modules\Plans\Init)->getPlans();
            if (count($plans) > 0) {
                if ($userActiveSubscriptionsCount > 0) {
                    $userActiveSubscription = $userActiveSubscriptions[0];
                    $userActiveSubscriptionId = $userActiveSubscription->ID;
                    $subscription = new WC_Subscription($userActiveSubscriptionId);
                    $subscriptionOrderId = $subscription->order->id;
                    $subscriptionOrder = wc_get_order($subscriptionOrderId);
                    $subscriptionOrderSum = intval($subscriptionOrder->get_total());
                    foreach ($plans as $key => $plan) {
                        if ($subscriptionOrderSum >= $plan['price']) {
                            unset($plans[$key]);
                        }
                    }
                }
                if ($userActivePlans[0]->plan->slug == 'free-trial' || ($userActiveSubscriptionsCount > 0 && count($plans) > 0)) {
                    $buttonActionText = 'Update You Plan';
                } else {
                    $buttonActionText = 'Buy Plan';
                }
            }
        } else {
            $plans = false;
        ...
        ?>
        <div class="pupuga-plan-proposition">
            <div class="pupuga-plan-proposition__item">
                <a href="<?php echo home_url('/pricing/') ?>" class="button pupuga-plan-proposition__button pupuga-plan-proposition__button--prices">View Stories Online Pricing</span></a>
            </div>
            <?php if ($plans) : ?>
                <?php foreach ($plans as $plan) : ?>
                    <?php if ($plan['price'] != 0) : ?>
                        <div class="pupuga-plan-proposition__item">
                            <a href="<?php echo get_permalink(wc_get_page_id('shop')); ?>?add-to-cart=<?php echo $plan['id'] ?>" class="button pupuga-plan-proposition__button pupuga-plan-proposition__button--<?php echo sanitize_title($plan['name']) ?>"><?php echo $buttonActionText ?> for <span class="pupuga-plan-proposition__salary"><?php echo $plan['currency'] . $plan['price'] . '&nbsp;/&nbsp;' . $plan['period'] ?></span></a>
                        </div>
                    <?php endif ?>
                <?php endforeach; ?>
            <?php endif ?>
            ...
            <table class="shop_table shop_table_responsive my_account_orders my_account_memberships">
                ...
                <tr>
                    ...
                    <?php foreach ($my_memberships_columns as $column_id => $column_name) : ?>
                        ...
                        ?>
                        <?php if ($column_id !== 'membership-next-bill-on') : ?>
                ...
                <?php foreach ($customer_memberships as $customer_membership) : ?>
                    ...
                    <?php if (!$customer_membership->get_plan()) {
                        continue;
                    } ?>
                        ...
                        <?php foreach ($my_memberships_columns as $column_id => $column_name) : ?>
                            <?php if ($column_id != 'membership-next-bill-on') : ?>
                                ...
                                <?php elseif ('membership-status' === $column_id) : ?>
                                    ...
                                    <td class="membership-status pupuga-status pupuga-status--<?php echo strtolower(wc_memberships_get_user_membership_status_name($customer_membership->get_status())) ?>" style="white-space:nowrap;" data-title="<?php echo esc_attr($column_name); ?>">
