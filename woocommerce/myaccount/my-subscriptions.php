    ...
    <div class="woocommerce_account_subscriptions">
        ...
        <?php if (!empty($subscriptions)) : ?>
            <table class="shop_table shop_table_responsive my_account_subscriptions my_account_orders">
                ...
                <?php
                $i = count($subscriptions);
                $reverseSubscriptions = array_fill(0, $i , null);
                foreach ($subscriptions as $subscription) {
                    $i = $i - 1;
                    $reverseSubscriptions[$i] = $subscription;
                }
                ?>
                <?php foreach ($reverseSubscriptions as $subscription) : ?>
                    <?php if (intval($subscription->get_total()) !== 0) : ?>
                        <tr class="order">
                            ...
                            <td class="subscription-status order-status pupuga-status pupuga-status--<?php echo strtolower(wcs_get_subscription_status_name($subscription->get_status())) ?>" data-title="<?php esc_attr_e('Status', 'woocommerce-subscriptions'); ?>">
                            ...
                            <td class="subscription-actions order-actions">
                                <a href="<?php echo esc_url($subscription->get_view_order_url()) ?>" class="button view">Manage Your Subscription</a>
